...
exports.handle = (req, res, fn, err) =>
  new Promise(async (resolve, reject) => {
    ...
    .then(val => {
      if (val == null) return send(res, res.statusCode || 204, null);
      if (val != undefined) send(res, res.statusCode || 200, val);
    })
...
exports.redirect = (res, target) => {
  res.statusCode = 302;
  res.setHeader('Location', target);
  res.end();
  return null;
};
exports.withRedirect = ({target = '', path = ''} = {}) => handler => async (
  req,
  res
) => {
  if (!res.redirect) {
    res.redirect = trgt => exports.redirect(res, trgt);
  }
  ...
  target = target && typeof target === 'function' ? await target(req) : target;
  if (!target) return await (handler && handler(req, res));
