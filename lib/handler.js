...
exports.handle = (req, res, fn) => new Promise(resolve => resolve(fn(req, res)))
.then(val => {
  if (val === null) {
    send(res, 204, null);
    return;
...
exports.buffer = (req, { limit = '1mb', enc } = {}) => Promise.resolve().then(() => {
...
exports.text = (req, {limit, enc} = {}) => exports.buffer(req, {limit, enc}).then(body => body.toString(enc));

exports.json = (req, opts) => exports.text(req, opts).then(body => parseJSON(body));

exports.compose = (...fns) => fns.reduce((f, g) => (...args) =>
  g ? f(g(...args)) : f(...args),
  fn => async (...args) => fn(...args)
);

exports.cors = options => handler => async (req, res) => {
  const DEFAULT_ALLOW_METHODS = ['POST', 'GET', 'PUT', 'PATCH', 'DELETE', 'OPTIONS'];
  const DEFAULT_ALLOW_HEADERS = ['X-Requested-With', 'Access-Control-Allow-Origin', 'X-HTTP-Method-Override', 'Content-Type', 'Authorization', 'Accept'];
  ...
  res.setHeader('Access-Control-Max-Age', '' + (maxAge || DEFAULT_MAX_AGE_24H_IN_SECONDS));
...
exports.query = options => handler => async (req, res) => {
  req.query = url.parse(req.url, true).query;
  return await handler(req, res);
};

exports.time = ({suffix}) => handler => async (req, res) => {
...
exports.log = ({json}) => handler => async (req, res) => {
  const handled = await handler(req, res);
  ...
  res.once('finish', () => {
    ...
    console.log(`${req.headers.host || '-'} ${req.url || '-'} ${res.statusCode || 0} ${req.method || '-'} ${req.headers['x-forwarded-for'] || req.connection.remoteAddress || '-'} ${req.headers['user-agent'] || '-'} ${req.headers['content-length'] || 0} ${res.getHeader('Content-Length') || 0}`);
  });

  return handled;
};

exports.interval = ({period}) => handler => {
